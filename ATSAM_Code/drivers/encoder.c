...
int8_t encoder_init(void) {
  ...
  if (PMC->PMC_WPSR & 1) {
    printf("Write protect couldn't be removed, writing to PMC clock for "
           "encoder failed");
    return 1;
  }

  // Disable PIO WP, for some reason it wont let us write to BMR, this is just a
  // test
  ...
  if (PIOC->PIO_WPSR & 1) {
    printf(
        "Write protect couldn't be removed, writing to PIO for encoder failed");
    return 1;
  }

  // CH0 clock must be enabled to access WPMR, disable WP, key "TIM" in ASCII
  TC2->TC_WPMR = TC_WPMR_WPKEY(0x504D43);
