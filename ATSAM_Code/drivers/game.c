...
uint8_t legacy_game(struct motor_device *motor_dev,struct pid_controller
*motor_pid){
        ...
        switch(game_state){
                ...
                case game_playing:{
                        ...
                        //Start motor
                        pwm_dir_and_speed(&motor_dev, &motor_pid, (-ctrl->joy_x)
* JOY_SENS);
                ...
                case game_ready:{
                        ...
                        CAN_MESSAGE gameover_msg = {0x2, 8,
{{ctrl->score},{ctrl->lifes},{0},{0},{0},{0},{0},{0}}}; can_send(&gameover_msg,
1);
...
uint8_t basic_game(struct game_config *config) {
  ...
  case game_playing:
    ...
    pwm_dir_and_speed(config->motor_dev, config->motor_pid,
                      (-config->joy->x) * JOY_SENS);
    ...
    if ((config->btn->L5) ||
        (simple_goal_detection())) { // Added a dedicated button to lose as the
                                     // IR on some of the motor boxes is broken
      printf("GOAL------------------------------------------------------\r\n");
      motor_stop(config->motor_dev);
