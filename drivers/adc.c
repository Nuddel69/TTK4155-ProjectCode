...
void ADC_init(void) {
    ...
    // Do NOT touch DDR/PORTA (PORTA is the external data bus and is handeled trough writing to and reading to xmem)
    // No need to set up PD6/PD7; XMEM controls WR/RD automatically when we read or write to memory allocted to the ADC
...
void ADC_test(void){
    ...
    printf("--ADC Test--\n\r");
    printf("CH0=%3d  CH1=%3d  CH2=%3d  CH3=%3d\r\n", data.channel[0]-128, data.channel[1]-128, data.channel[2]-128, data.channel[3]-128);
