...
int8_t CAN_write(struct can_device *dev, uint8_t address, struct CAN_frame data_frame){
        ...
        for (uint8_t i = 0; i < 8; i++){
            uint8_t adr = 0x00110110 + i;
            MCP2515_write(dev, adr, &data_frame.data);
			}
...
int8_t MCP2515_write(struct can_device *dev,uint8_t addr,uint8_t *data){
    ...
    spi_push(dev, MCP2515_OP_WRITE, NULL); //Set operation
    spi_push(dev, addr, NULL);             // Set address
    spi_push(dev, data, NULL);             // Write data
    return 0;
}
...
int8_t MCP2515_request_to_send(struct can_device *dev,uint8_t tx_buf_num){
    ...
    spi_push(dev, MCP2515_OP_RTS_BASE | (1 << tx_buf_num),NULL);
