...
void xmem_init(void){
    ...
    //The pins in range PC7-PC4 is reserved for JTAG, and XMBK to stop pins from floating see pg32 datasheet
    SFIOR = (SFIOR & ~((1<<XMM1)|(1<<XMM0)))|((1<<XMM2)|(1<<XMBK));


    //Extended MCU Control Register pg30 datasheet
    //Setting SRL2-SRL0 as 1 gives the sectors lower sector= 0x1100-0xDFFF & high sector =0xE000-=xFFFF
    //Setting SRW01 & SRW00 as 1 gives waitstate 2 cycles before writing and 1 cycle before sending new address, see pg 31
    EMCUCR = (1<<SRW01)|(1<<SRW00)|(1<<SRL2)|(1<<SRL1)|(1<<SRL0);

    //MCU Control Register pg30
    //Same as EMCUCR but for upper sector, set to wait 1 cycle for read & write, see table at pg31
...
void SRAM_test(void) {
    ...
    for (uint16_t i = 0; i < ext_ram_size; i++) {
        ...
        uint8_t retrieved = ext_ram[i];
        if (retrieved != value) {
            printf("Retrieval phase error: ext_ram[%4u] = 0x%02X (should be 0x%02X)\n",
                   i, (unsigned)retrieved, (unsigned)value);
            retrieval_errors++;
        }
    }
